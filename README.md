...
- 主程序启动时及每 6 小时做一次会话体检与自动续期。
- 新凭据通过登录检查并原子写入文件后，才确认旧凭据失效；确认失败会保留待确认口令，重启后继续处理。
- `86095` 会重新获取实时 CSRF 并重试一次，不会直接删除刷新口令。已过期或被强制下线的会话仍需重新扫码。
...
- **截图失败**：后台每分钟检查失败任务，按 30 秒起、最长 1 小时的退避间隔重试；重启后可继续，不补发首次监听导入的历史推文。
